//=========================================================
//
//This is a prog in the MutualInfo 1.0 package written by
// Hanchuan Peng.
//
...
//
// make -f estmutualinfo.makefile
// g++ estmutualinfo.cpp -o main.o -lm
//
// by Hanchuan Peng
// 2005-08-01
// 2005-10-17
...
DataTable *readData (char filename[], char sep, int b_readName);
...
{
  printf
    ("Usage: mutualinfo -i <dataset> -t <threshold> [optional arguments]\n");
  printf
...
DataTable *
readData (char filename[], char sep, int b_readName)
{
...
//  printf ("\ninitial input len=%d\n", len);
  ...
  while ((nbyteread = getline (&line, &len, fid)) != -1)
    {
...
//        default:        abort ();
  ...
  if (select_method == MID)
    printf
      ("\ninputfile=\"%s\" threshold=mu+/-%4.2f*sigma #fea=%d selection method=MID\n",
       input_dfile, t_discretize, nfea);
  else if (select_method == MIQ)
    printf
      ("\ninputfile=\"%s\" threshold=mu+/-%4.2f*sigma #fea=%d selection method=MIQ\n",
       input_dfile, t_discretize, nfea);
  else
    {
  ...
  DataTable *myData = readData (input_dfile, ',', 1);
...
//  printf ("nsample read=%d\nn_var read=%d\n", myData->nsample, myData->nvar);
  ...
  if (long (t_discretize) != 9999)
    myData->discretize (t_discretize);	//threshold at 1 for the z-scored data
