...
// 2008-6-23: remove main and readdata -- to be used by java interface.
...
printPaperInfo()
{
  ...
  printf( "     algorithm were developed by Hanchuan Peng <penghanchuan@yahoo.com> (http://hpeng.net) for\n");
  printf( "     the paper \n");
...
readData (char filename[], char sep, int b_readName, long nmaxvar, long nmaxsample)
{
	...
	while ((nbyteread = getline (&line, &len, fid)) != -1)  //cygwin on windows does not support this function
    {
...
{
  cout << "Begin mRMR_selection()" << endl;             
  long *feaInd = 0;
...
//	printf ("%d \t %d \t %s \t %5.3f\n", i, int (poolInd[i]),
  ...
  printf ("\n*** mRMR features *** \n");
  printf ("Order \t Fea \t Score\n");
   printf ("%d \t %d \t %5.3f\n", 0 + 1, feaInd[0],  mival[1]);
