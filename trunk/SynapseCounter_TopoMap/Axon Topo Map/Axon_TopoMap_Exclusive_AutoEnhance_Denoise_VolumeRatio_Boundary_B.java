 ...
 *  
 * 12/25/2013
 *  Add a denoise in neuropil boundary, with mainly affects boundary analysis but also slightly affect other measures
 */
...
public class Axon_TopoMap_Exclusive_AutoEnhance_Denoise_VolumeRatio_Boundary_B implements PlugIn, AdjustmentListener, TextListener {
    static final String pluginName = "Axon_TopoMap_Exclusive_AutoEnhance_Denoise_VolumeRatio_Boundary_B";
   ...
   int  leafSizeNeuropil = 5; //Should I adjust leaf size to overcome the speckly noise(up to 28?
	...
	//added 12/25/2013, denoise to remove little things.
	if (show_mask)	combinedImage.duplicate().show();
	IJ.run(combinedImage, "Median...", "radius=1");
	//int minSizeTh = 5;
	//NeuropilDenoiser.removeSmallComponents(combinedImage.getProcessor(), Height, Width, minSizeTh, 0);
	...
	//IJ.log("Territory Proportion:"+res[2]/res[3]);
    IJ.log("Boundary: dorsal relative distance: " + res[8]/res[10]); //boundaryMetrics[0]);
	IJ.log("Boundary: ventral relative distance: " + (1-res[9]/res[10])); //boundaryMetrics[1]);
