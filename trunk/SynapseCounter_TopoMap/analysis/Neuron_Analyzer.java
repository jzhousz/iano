...
public class Neuron_Analyzer implements PlugIn {
	public void run(String arg) {
		...
		//Create a new copy of the image processor so that the original is not altered
		...
		//Create binary image to use as guide
		ImageProcessor binaryIP = ip.duplicate();
		HistogramEq.run(new ImagePlus("EQ", binaryIP), 63, 255, 3.00f, null, null);		
		//Use green channel if color image
		byte[] binpixels= new byte[width*height];
		if (binaryIP instanceof ColorProcessor) {			
			byte[] tmppixels= new byte[width*height];			
			((ColorProcessor)binaryIP).getRGB(tmppixels,binpixels,tmppixels);
			binaryIP = new ByteProcessor(width, height, binpixels, null);
		}
		...
		//Histogram equalization for soma detection
		//HistogramEq.run(new ImagePlus("EQ", ip), 127, 256, 3.00f, null, null);
		
		byte[] pixels= new byte[width*height];
		//Use green channel if color image		
		if (tempIP instanceof ColorProcessor) {			
			byte[] tmppixels= new byte[width*height];			
			((ColorProcessor)tempIP).getRGB(tmppixels,pixels,tmppixels);
			tempIP = new ByteProcessor(width, height, pixels, null);
		}
		
		//For the case requiring equalization - namely mutants
		HistogramEq.run(new ImagePlus("EQ", ip), 127, 256, 3.00f, null, null);
		...
		if (ip instanceof ColorProcessor) {			
			byte[] tmppixels= new byte[width*height];			
			((ColorProcessor)ip).getRGB(tmppixels,pixels2,tmppixels);
			ip = new ByteProcessor(width, height, pixels2, null);
		}
		...
		//Edge detection
		final Image img = Image.wrap(new ImagePlus("Soma", tempIP));
		...
		//Hough transform
		tempIP = newimg.imageplus().getProcessor().convertToByte(true);	        
		HoughTransform ht = new HoughTransform(10, 10, 2, width, height, width, 0, 0);
		ImageProcessor hip = tempIP.duplicate();
		try {
			hip = ht.run(hip);
		}
		catch(Exception ex) {
		...
		//Check if there is any object left, if not repeat the process by equalizing first
		...
		if(!detected) {
			//Edge detection
			final Image img2 = Image.wrap(new ImagePlus("Soma", ip));
			Image newimg2 = new FloatImage(img2);
				
			final Aspects aspects2 = newimg2.aspects();
			
			//if (!FJ_Options.isotropic) 
				newimg2.aspects(new Aspects());
			
			final Edges edges2 = new Edges();
			newimg2 = edges2.run(newimg2,1.0,false);
			newimg2.aspects(aspects);
			
			//Hough transform
			ip = newimg2.imageplus().getProcessor().convertToByte(true);	        
			//HoughTransform ht = new HoughTransform(10, 10, 2, width, height, width, 0, 0);
			hip = ip.duplicate();
			try {
				hip = ht.run(hip);
		...
		//Dilate to edge
		for(Integer key : blobIPs.keySet())
			DilateToEdge.somaRun(new BinaryProcessor((ByteProcessor)binaryIP), blobIPs.get(key));
		...
		//Remove axon part		
		//Remove noise with despeckle : median filter with radius 1
		filter.rank(binaryIP, 1, RankFilters.MEDIAN);
		new ImagePlus("Binary2", binaryIP).show();
		
		removeAxon(somaip, maskIP, new BinaryProcessor((ByteProcessor)binaryIP));
		maskIP = t.run(somaip);
