...
public class Axon_TopoMap_Inclusive implements PlugIn, AdjustmentListener, TextListener {
    static final String pluginName = "Axon_TopoMap_Inclusive";
	...
	gd.addCheckbox("Enhance the neuropil channel", enhanceNeuropil);
	...
	show_mask = gd.getNextBoolean();     show_mask_default = show_mask;
        ...
        IJ.register(Axon_TopoMap_Inclusive.class); // static fields preserved when plugin is restarted
        //Reset the threshold
        ip.resetThreshold();
        img.updateAndDraw();
        return true;
   ...
   float calcTerritoryPortion(int axonMaxZ, int axonMinZ, int neuropilMaxZ, int neuropilMinZ, int limit)
	...
	return res;
     ...
     //get boundary by linking neighbors
