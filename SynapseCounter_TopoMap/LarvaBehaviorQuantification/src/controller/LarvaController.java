...
//	private int lastTrackFrame = 0;
	...
	public LarvaController(OptionComponent optionComponent, int procIndex, int totalProcNum)
	{
		...
		showSysStatus("[System] Deleting all statistical segmentation file: plugins/Larva/train_test.", Color.gray);
		File dir = new File("plugins/Larva/train_test");
		FileManager.purgeDirectory(dir);
	...
	public void run()
	{
		...
		// -------------------------------------------
		showSysStatus("[System] Fixing larva that was not segmented correctly...", colorSys);
		...
		if(prop.getUse_pca().equals("true"))
		{
			...
			for (Larva larva : listLarva)
			{
				...
				// if the larva is have invalid body area and skeleton length
				if(!larva.getIsValid() &&  !(larva.getIsOverSize() || larva.getIsOverSkeleton()) )
				{
					FileManager.copyFile(dirImageDebug + "dBinary_Advanced/" + "BigObject3_Before_Centered_" + larva.getFrameId() + ".jpg"
							, Trainer.imageDiscardDir + "BigObject3_Before_Centered_" + larva.getFrameId() + ".jpg");
				}
			}
...
//		int j = 5/0;
		...
		showSysStatus("[System] Deleting all temporary files.", Color.gray);
...
//		showSysStatus("[System] Deleting all debugging images file: " + dirImageDebug + ".", Color.gray);
		...
		if (!PropertyManager.getBool(prop.getOutput_debug()))
		{
			...
			FileManager.deleteDir(new File(dirImageDebug + "dBinary_Advanced/"));
			FileManager.deleteDir(new File(dirImageDebug + "dSkeleton_Before_Centered/"));
		}
		
//		showSysStatus("[System] Deleting all annotation images file: " + dirImageDebug + ".", Color.gray);
		
		if (!PropertyManager.getBool(prop.getOutput_animated_image()))
			FileManager.deleteDir(new File(dirImageDebug + "aAnnotation/"));

		showSysStatus("[System] Deleted all temporary files!", Color.gray);
	...
	public void retrieveLarvaInfo(ImagePlus imagePlusBinary, Larva larva, Larva larvaPrevious, int frameId, int nth)
	{
		...
		if (larva.getLarvaPrevious() != null)
		{
			String imagePath = dirImageDebug + "dBinary_Advanced/" + "BigObject3_After_Centered_"
					+ Integer.toString(frameId - 1) + ".jpg";
