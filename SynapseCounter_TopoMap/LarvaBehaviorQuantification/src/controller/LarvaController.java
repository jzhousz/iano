...
public class LarvaController implements Runnable 
{
	...
	*/
	public LarvaController(OptionComponent optionComponent, int procIndex, int totalProcNum)
	{
		...
		int aviHeight = stackFrame.getProcessor(1).getHeight();
		int aviWeight = stackFrame.getProcessor(1).getWidth();
		imageArr = new int[aviHeight][aviWeight];
...
//			{
//				ImageSaver.saveImagesWithPath(dirImageDebug+"dBinary_Advanced/", "BigObject3_Before_Centered_" + frameId, imagePlusBigObj);
//			}
			...
			// Save the coordinate of top left corner of the ROI on avi full frame
...
//		int yy = 10 / 0;
		...
		// ------------------ new rolling analysis approach (forward-backward approach) -------
		int frequent = 5;
		ArrayList<LarvaeAndRolls> listLarvaeRolls = HelperManager.getRollingFrames(listLarva, frequent);
		
		// save the rolling tag and number of rolls in rolling sections to the listLarva
		...
		if( PropertyManager.getBool( prop.getOutput_video() )  )
		{
			...
			showSysStatus("(generateVideo) Generating video beginning from frame " + firstTrackFrame + ".", colorSys);
...
//		(new SummaryManager(listLarva)).show();
		...
		showSysStatus("[System] The process has been completed!"
				+"\n                                     "+"Total Time Needed for the process: " 
					+ HelperManager.getDuration(date) +".", Color.green);
		
//		showSysStatus("[System] Total Time Needed for the process: " + HelperManager.getDuration(date) +".");
	...
	*/
	void generateAnnotationImages()
	{
		...
		showStatus("\nGenerating frames from " + listLarva.get(0).getFrameId() 
				+ " to " + listLarva.get(listLarva.size() - 1).getFrameId() + ":\n", colorSys);
...
//				{
//					showSysStatus("Yao throws exception: " );
