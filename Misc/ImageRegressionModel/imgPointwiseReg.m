function [bhat ahat yhat] = imgPointwiseReg(Y, X, fromlevel, tolevel)
%
% target image of (dimx*dimy)   Y  e.g. eve
% source image of dimx*dimy.  X
% fromlevel: 4, tolevel 5.  (if 2 to 3, slower with more bases)
%imgPointwiseReg(eve, hb_same, 4, 5)
%bhat: regression coefficient matrix 
%ahat: constant
%yhat: predicted image using X and coefficients (can calculate residue)
%
dimx = size(Y,1);
dimy = size(Y,2);
...
r1=norm(bhat.*X+ahat-Y,'fro')/norm(Y,'fro')
r2=norm(ahat-Y,'fro')/norm(Y,'fro')

% mask images ... not related to this project
