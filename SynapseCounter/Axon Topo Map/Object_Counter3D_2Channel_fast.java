...
public class Object_Counter3D_2Channel implements PlugIn, AdjustmentListener, TextListener, Runnable {
    ...
    public static int two_channel_proximity_default = 2;  //the smaller, the restricter (may return smaller count)
    ...
    public boolean setupGUI(String arg) {
  	...
  	gd.addNumericField("two channel proximity check range: ",two_channel_proximity_default,0);
	...
	two_channel_proximity = (int) gd.getNextNumber();  two_channel_proximity_default = two_channel_proximity;
    ...
    // are considered.
    //
    void setMaskUsing2Channels(int index1, int index2, int ThrVal, int ThrVal2, ImageStack stack, boolean[] thr)
    {
    	 int z, y, x;
         int arrayIndex=0;
	 int pict2val;
    ...
    boolean neighboring(int z, int y, int x, ImageStack stack, int ThrVal2, int index2)
    { 
	  ...
	  ImageProcessor ip;
    	for(r = 1; r <= two_channel_proximity_default; r++)
    	{
    	  if(withinBounds(x,y, z-r))
    	  {
	       ...
	       PixVal = ip.getPixel(x,y,null);
           pict2val = PixVal[index2];
    	   if(pict2val > ThrVal2) return true;
    	  }
    	  if(withinBounds(x,y, z+r))
     	  {
           ip = stack.getProcessor(z+r);
	...
	IJ.log("thr allocated");
        tag=new short [Height*Width*NbSlices];   //2 byte, upto 32,767 objects
